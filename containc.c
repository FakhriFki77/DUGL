...
#define DEFAULT_DFILEBUFFER_BUFFER_SIZE	128*1024
...
unsigned int GetBytesDFileBuffer(DFileBuffer *ptr, void *buff, unsigned int bytesToGet) {
	/* TODO */
	return 0;
}
...
unsigned int GetLineDFileBuffer(DFileBuffer *ptr, char *line, unsigned int maxLineSize) {
	unsigned int 	sizeLine = 0,
					bytesCopied = 0,
					i = 0,
					startPos = ptr->m_curPos;
	bool 			endLine = false;

	if (ptr->m_bytesInBuff == 0) {
		if (ptr->m_EOF)
			return 0;
		ReadChunkDFileBuffer(ptr);
		if (ptr->m_bytesInBuff == 0)
	...
	for (i=0; i <maxLineSize-1;i++) {
		if (ptr->m_buffRead[ptr->m_curPos] == '\n'/* || m_buffRead[m_curPos] == '\r'*/) {
			ptr->m_curPos++;
			ptr->m_bytesInBuff--;
			// check if next byte too is one of line terminator
			if (ptr->m_bytesInBuff < 1) {
				SDL_memcpy(&line[bytesCopied], &ptr->m_buffRead[startPos], sizeLine - bytesCopied);
				bytesCopied = sizeLine;
				ReadChunkDFileBuffer(ptr);
			}
			if (ptr->m_bytesInBuff > 0 && (/*m_buffRead[m_curPos] == '\n' ||*/ ptr->m_buffRead[ptr->m_curPos] == '\r')) {
				ptr->m_curPos++;
				ptr->m_bytesInBuff--;
			}
			endLine = true;
		...
		if (ptr->m_bytesInBuff > 1) {
			ptr->m_curPos++;
			ptr->m_bytesInBuff--;
		} else if (ptr->m_bytesInBuff == 1) {
			memcpy(&line[bytesCopied], &ptr->m_buffRead[startPos], sizeLine - bytesCopied);
			bytesCopied = sizeLine;
			startPos = 0;
			if (!ptr->m_EOF) {
				ReadChunkDFileBuffer(ptr);
...
bool IsEndOfFileDFileBuffer(DFileBuffer *ptr) {
	return ptr->m_EOF && (ptr->m_bytesInBuff == 0);
};
